...
namespace ButtonEmulator
{
    public partial class Form1 : Form
    {
        ...
        private void _buttonStartPWM_Click(object sender, EventArgs e)
        {
            if (!isConnected)
            {
            ...
            _ = int.Parse(TimeBox.Text);
            int dutyCycle = int.Parse(DutyCycleBox.Text);

            SendPWMCommand(dutyCycle);

            // сделать таймер для отключения шим сигнала
        }
        private void _buttonStopPWM_Click(object sender, EventArgs e)
        {
            if (!isConnected)
            {
                MessageBox.Show("COM порт не найден");
